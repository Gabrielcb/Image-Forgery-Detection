...
def np_im2patch(im, pShape,pStride=None):
    ...
    pad = np.array( np.add( np.multiply ( np.ceil( np.divide( np.subtract( imShape, np.subtract(pShape, pStride) )  , pStride, dtype=float) ),pStride),    np.subtract(pShape, pStride)) - imShape,dtype=int)
    pad = pad[0:2]
    assert((pad >= 0).all())
